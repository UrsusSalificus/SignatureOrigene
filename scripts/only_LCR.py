...
def extract_features (records, species_table, output, id_column, start_column,
                      end_column):
    ...
    with open(species_table, 'r') as feature_table, open(output, 'w') as outfile:
        ...
        for each_record in range(len(records)):
            ...
            for each_range in all_ranges:
                LCR_only += records[each_record].seq[each_range[0]:each_range[1]]

            new_record = SeqRecord(seq=LCR_only, id=records[each_record].id)
            LCR_records.append(new_record)

        # Write the new list of records
        SeqIO.write(LCR_records, outfile, "fasta")
