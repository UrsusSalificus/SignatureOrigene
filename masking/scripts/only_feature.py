...
def extract_feature(records, species_table, output, id_column, feature_column, feature_type, strand_column,
    ...
    with open(species_table, 'r') as feature_table, open(output, 'w') as outfile:
        ...
        for each_record in range(len(records)):
            ...
            while actual_line[feature_column] != feature_type or actual_line[strand_column] != '+':
                actual_line = feature_table.readline().split('\t')
                ...
                if actual_line[feature_column] == feature_type and actual_line[strand_column] == '+':
                    ...
                    if int(actual_line[start_column]) <= int(precedent_line[end_column]):
                        ...
                        if int(actual_line[end_column]) > int(precedent_line[end_column]):
                            precedent_range = set(range(int(precedent_line[start_column]), int(precedent_line[end_column])))
            ...
            for each_range in all_ranges:
                feature_only += records[each_record].seq[each_range[0]:each_range[1]]

            new_record = SeqRecord(seq = feature_only, id = records[each_record].id)
            feature_records.append(new_record)

        # Write the new list of records
        SeqIO.write(feature_records, outfile, "fasta")

# Fetch all the records from this species fasta
records = fetch_fasta(species_genome)

# Particularities of the feature table (e.g. which column (the numbers) contains what information):
id_column = 6
feature_column = 0
