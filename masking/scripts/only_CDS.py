...
""" Masking to only have Coding Sequences (CDS) nucleotides
"""
    ...
    with open(species_table, 'r') as feature_table, open(output, 'w') as outfile:
        ...
        for each_record in range(len(records)):
            ...
            for each_range in all_ranges:
                CDS_only += records[each_record].seq[each_range[0]:each_range[1]]

            new_record = SeqRecord(seq = CDS_only, id = records[each_record].id)
